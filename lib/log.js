...
function log (req) {
  ...
  req.on('request', function (req, _options) {
    ...
    if (is.req.name === 'enabled') {
      ...
      var headers = {}
      extend(true, headers, _options.headers)

      var h = Object.keys(_options.headers).map(function (key) {
        return key.toLowerCase()
      })
      Object.keys(req._headers).filter(function (key) {
        return h.indexOf(key) === -1
      })
      .forEach(function (key) {
        headers[key] = req._headers[key]
      })
